  ...
  @:glFragmentSource('
   ...
   // uniform sampler2D noiseTex;
    ...
    void main()
    {
      ...
      video.r += 0.08*getVideo(vec2(0.75)*vec2(x+0.025, -0.027)+vec2(uv.x+0.001,uv.y+0.001)).x;
      video.g += 0.05*getVideo(vec2(0.75)*vec2(x+-0.022, -0.02)+vec2(uv.x+0.000,uv.y-0.002)).y;
      video.b += 0.08*getVideo(vec2(0.75)*vec2(x+-0.02, -0.018)+vec2(uv.x-0.002,uv.y+0.000)).z;

      video = clamp(video*vec4(0.6)+vec4(0.4)*video*video*vec4(1.0),0.0,1.0);
      if(vignetteMoving)
...
class VHSEffect extends Effect
{
	...
	public function new(glitchRange:Float, intensity:Float)
	{
                shader = new VHSShader();
		shader.iTime.value = [0];
		shader.noisePercent.value = [0.0];
		shader.range.value = [glitchRange];
		shader.offsetIntensity.value = [intensity];
		shader.iResolution.value = [Lib.current.stage.stageWidth, Lib.current.stage.stageHeight];
		PlayState.instance.shaderUpdates.push(update);
	}
